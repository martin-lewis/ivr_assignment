...
class image_converter:
  ...
  # main callback, runs all tasks
  # uncomment the task you want to run
  def callback1(self,data):
    try:
      self.images_buffer["camera1"].append(self.bridge.imgmsg_to_cv2(data, "bgr8"))
    except CvBridgeError as e:
      print(e)
    
    self.images_buffer["camera1_received"] = True

    if self.images_buffer["camera2_received"]:
      self.images_buffer["camera1_received"] = False
      self.images_buffer["camera2_received"] = False

      self.cv_image1 = self.images_buffer["camera1"].pop(0)
      self.cv_image2 = self.images_buffer["camera2"].pop(0)
      self.main()

  #Additional Callback used to get the image from the other cameras (camera2)
  def callback2(self,data):
    try:
      self.images_buffer["camera2"].append(self.bridge.imgmsg_to_cv2(data, "bgr8")) #New image stored under self.cv_image2
    except CvBridgeError as e:
      print(e)

    self.images_buffer["camera2_received"] = True

    if self.images_buffer["camera1_received"]:
      self.images_buffer["camera1_received"] = False
      self.images_buffer["camera2_received"] = False
  ...
  def __init__(self):
    ...
    # used to synchronize the images from the two cameras
    self.images_buffer = {
      "camera1":[],
      "camera2":[],
